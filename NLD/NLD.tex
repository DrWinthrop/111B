...
\signatures \hyperlink{DFT Window Units}{1} \hyperlink{Cob Web Program}{2} \hyperlink{Liapunov Exponent Program}{3} \hyperlink{Henon Map}{4} \hyperlink{Bouncing Ball and PN-Junction Circuits}{5} 
...
\begin{figure}[h]
    ...
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/NLD_3516.jpg}{\includegraphics[width=\linewidth,keepaspectratio]{images/NLD_3516.jpg}}
    ...
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/NLDDAQ.jpg}{\includegraphics[width=\linewidth,keepaspectratio]{images/NLDDAQ.jpg}}
    \caption{DAQ box. See larger image \href{http://experimentationlab.berkeley.edu/sites/default/files/images/NLDDAQ.jpg}{\textbf{here}}}
    \end{minipage}
    \begin{minipage}{0.48\textwidth}
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/NLDrack.jpg}{\includegraphics[width=\linewidth,keepaspectratio]{images/NLDrack[1].jpg}}
    \caption{The NLD apparatus wired for bouncing ball. See larger image \href{http://experimentationlab.berkeley.edu/sites/default/files/images/NLDrack.jpg}{\textbf{here}}}
    \end{minipage}
\end{figure}
...
\begin{thebibliography}{}
    ...
    \bibitem{Wells} L. Wells and J. Travis \emph{LabVIEW for Everyone, }Prentice Hall PTR (Available from \LabEngineer).
\end{thebibliography}
\endgroup
...
\textbf{NOTE for future reference: Once we switch over to Windows 10, we will also be switching over to \emph{LabVIEW 2015}, but most other experiments will be using \emph{LabVIEW 2016}. The distinction between versions is made here, because the custom built VI's call on the CALLCORR.VI sub-vi, which uses a 32 bit file that requires a 32 bit version of LabVIEW to run. When using Windows 10, don't use \emph{LabVIEW 2016}, but rather use \emph{LabVIEW 2015}, because this version is 32 bit and can run the necessary sub VI's.}
...
Thus the diode may be modeled as a voltage-dependent capacitor connected in parallel with an ideal diode (a voltage-dependent current-source), giving the equivalent circuit on the right of Figure \ref{fig:Circuit}.
...
If one could measure all the dynamical variables of a system simultaneously, the path $\vec p(t)$ could be plotted easily. However, in many cases this is not possible and, moreover, it is not necessary in most cases. Look ahead to the reconstructed phase plots of Figures \ref{fig:my_label} to \ref{fig:Chaotic}. These plots were made by sampling the diode current only. Rather than measuring the complete set of variables $(I, V_d, \theta)$, the trajectory is available in \emph{each} variable (i.e. we could have sampled the diode voltage and come up with similar plots).
...
Figure \ref{fig:my_label}- Figure \ref{fig:Chaotic} show reconstructed phase-space trajectories, $ \vec p(t) $, that correspond to the circuit's operation in Figure \ref{fig:Period15}- Figure \ref{fig:Nldimage019}. The plots were made using the embedding technique described in the previous section, so the plot axes are $ (I_n, I_{n+1}, I_{n+2}) $. We speculate that $\theta$ revolves around the line $z = y = x$, and that the other two state variables, V and I, correspond loosely to the distance along that line and the distance from that line. The graphs are all stereo-optic. Try holding them at arm's length (or set them on a desk as you stand over them) and letting your eyes focus at infinity. Let two of the double images overlap so that you can see three images. The middle one, if you get it to match up just right, will look like it's 3D.
...
For small values of the parameter $V_{os}$, the trajectory is a stable orbit. For larger value (after bifurcation to period 2) the orbit is still stable, but requires \emph{two} cycles of the driving oscillator to return to its initial point (Figure \ref{fig:perioddoubling}). These trajectories are a subclass of a more general object called an \emph{\textbf{attractor}}: The subset of state space toward which a dynamical system evolves. Because they are periodic (the paths are closed cycles), the simple attractors of Figure \ref{fig:my_label}-Figure \ref{fig:sameasabove} are known as \emph{\textbf{limit-cycles}}.

\begin{figure}[H]
    \centering
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/Nldimage044.gif}{\includegraphics[width=0.40\linewidth,keepaspectratio]{images/Nldimage044.png}}
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/Nldimage045.gif}{\includegraphics[width=0.40\linewidth,keepaspectratio]{images/Nldimage045.png}}
...
\begin{figure}[H]
    \centering
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/Nldimage046.gif}{\includegraphics[width=0.40\linewidth,keepaspectratio]{images/Nldimage046.png}}
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/Nldimage047.gif}{\includegraphics[width=0.40\linewidth,keepaspectratio]{images/Nldimage047.png}}
    \caption{Same as above after another bifurcation.}
    \label{fig:sameasabove}
\end{figure}
...
\begin{figure}[H]
    \centering
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/Nldimage048.gif}{\includegraphics[width=0.40\linewidth,keepaspectratio]{images/Nldimage048.png}}
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/Nldimage049.gif}{\includegraphics[width=0.40\linewidth,keepaspectratio]{images/Nldimage049.png}}
...
Once you're ready to begin, copy all files to your my documents folder chaos. (Note: go to C:\textbackslash support\textbackslash NLD2013 for the NLD.llb file and copy all into your folder manually. Now start LabView (\textbf{Again, use \emph{LabVIEW 2016} for this part}) from the Windows Start menu. Your first assignment is to make a simple VI. Investigate sampling by putting a \emph{waveform graph} and a \emph{waveform chart} on the front panel. These are located under \textbf{Functions-}F which can be accessed by right clicking in an empty space in ``Block Diagram''.(in the same way the \textbf{Controls} can be accessed by right clicking in an empty space in ``Front Panel''.
...
\]
The $x$-values form a series $\{m_0,m_0,m_1,m_1,m_2,m_2,\cdots,m_{n-1},m_{n-1}\}$ which we may form by ``interleaving'' the $x_i$ array with itself and removing the last two elements. Fortunately, LabView has a function that interleaves arrays and a function that takes an array's subset.
...
\checkpoint{Cob Web Program}{Call over a GSI and Professor and show them your program. Explain what you have done.}
...
\checkpoint{Henon Map}{Take a look at the Henon Map. Play with the non-linear parameter between 0 and about 1.4. What is that funky thing in the Return Map? Look at the Henon map's bifurcation program as well. Discuss what you are thinking with a GSI or Professor.}

% \textbf{This is a Checkpoint: Take a look at the Henon Map. Play with the non-linear parameter between 0 and about 1.4. What is that funky thing in the Return Map? Look at the Henon map's bifurcation program as well. Discuss what you are thinking with a GSI or Professor.}
...
\begin{figure}[H]
    \centering
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/NLD_PN_DIAGRAM.png}{\includegraphics[width=0.6\linewidth]{images/NLD_PN_DIAGRAM.png}}
...
\begin{figure}[h]
    \centering
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/Nldimage128.gif}{\includegraphics[width=0.7\linewidth]{images/Nldimage128.png}}
...
\begin{itemize}
    \item Combine a terminator on a T and connect the SRS DS345 with a sine wave out to the DRIVE input on the panel.
...
\begin{figure}[h]
    \begin{minipage}{0.48\textwidth}
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/NLD_BALL_DIAGRAM.png}{\includegraphics[width=\linewidth,keepaspectratio]{images/NLD_BALL_DIAGRAM.png}}
    \caption{Connections for Bouncing Ball Circuit \\ to the computer.}
    \label{fig:ConnectionsForBouncingBallCircuitToComputer}
    \end{minipage}
    \begin{minipage}{0.49\textwidth}
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/Bouncingball.jpg}{\includegraphics[width=\linewidth,keepaspectratio]{images/Bouncingball.jpg}}
    \caption{Front panel of Nldgen.vi in \\ LabVIEW 2015 for Bouncing Ball.}
    \end{minipage}
\end{figure}
...
\checkpoint{Bouncing Ball and PN-Junction Circuits}{What is fundamentally different about the chaotic behavior of the bouncing ball circuit when compared to the PN-Junction? (Hint: What would the bifurcation diagram for this circuit look like? Does it even have one?) Discuss what you notice with a Professor or GSI.}
...
\begin{itemize}
    \item As an introductory exercise, do the following: Run NLSIM.VI and CALCCORR.VI. Select ``Random number generator.'' Choose 2048 points and press the ``Send data to the info-dimension VI'' button. 
    \item Press ``Calculate'' in CALCORR. When the calculation is done, use the graph cursors to estimate the slope of each line. How does each slope compare to the line's embedding-dimension?
\end{itemize}
...
\begin{figure}[H]
    \centering
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/Nldimage140.gif}{\includegraphics[width=0.75\linewidth]{images/Nldimage140.png}}
    \caption{\textbf{NLDBifur}}
\end{figure}

\begin{figure}[H]
    \centering
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/Nldimage139.gif}{\includegraphics[width=0.75\linewidth]{images/Nldimage139.png}}
    \caption{\textbf{Henon Bifur}}
\end{figure}

\begin{figure}[H]
    \centering
    \href{http://experimentationlab.berkeley.edu/sites/default/files/images/Nldimage138.gif}{\includegraphics[width=0.8\linewidth]{images/Nldimage138.png}}
