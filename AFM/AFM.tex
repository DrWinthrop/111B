...
\section{Description of Atomic Force Microscope}

Atomic Force Microscopy (AFM) is a new and relatively cheap method of imaging objects from the nano- to micron-scale.  Surfaces can be scanned in 3D with an AFM, which provides a quick and easy way to measure dimensions, roughness, and many other material characteristics in scientific fields across the board. The provider of our commercial AFM is called the AFMWorkshop, located at \href{http://www.afmworkshop.com/}{\textbf{www.afmworkshop.com}}. 
%TT/AFM units are very good for student laboratories at a reasonably affordable price. They are very helpful and will setup the unit.

In this experiment, you will learn how to use an AFM and take scans of about 5 different samples to get an understanding of the wide range of AFM applications. You will learn how to make precise measurements and how to improve your environment to obtain clearer scans. You will also use the AFM to calculate force-distance curves and measure Boltzmann's constant. \emph{\textbf{Also remember NOT to clean the samples; doing so will destroy them.}}
...
\signatures  \hyperlink{Laser Alignment}{ 1}  \hyperlink{Tip Settings}{ 2} \hyperlink{Chosen Experiment}{ 3} \hyperlink{Noise Floor}{ 4} \hyperlink{Boltzmann's Constant}{ 5}
...
In this lab, you will operate an atomic force microscope which is used to image 3D surfaces on the angstrom to micron level. AFM is relatively inexpensive and is used in many different fields of science including physics, chemistry, and biology.

\subsection{What is an Atomic Force Microscope?}

An atomic force microscope images objects on the scale of angstroms to microns through physical scanning, rather than optical methods (like conventional microscopes). The basic mechanism of the AFM is shown in Figure \ref{fig:BasicMechanism}. A tip attached to a cantilever interacts with a surface. Light from a laser is bounced off from the cantilever into a photodetector. From this signal, one measures the deflection of the cantilever. A sample is placed on a special stage underneath the tip. The stage contains a \hyperref[subsubsec:Piezo]{piezoelectric} driver that is controlled by software. In particular, it may be made to wiggle around in a raster scan pattern, as well as engage in feedback with the cantilever (constant-force mode). Variations in the height of the sample cause deflections of the cantilever, which are then picked up by the photodetector. Through this lab, you should gain a good understanding of how an AFM works, the types of data you can get from it, how you can use this data to make meaningful conclusions, and the limitations of the system.
...
\begin{figure}[h]
    ...
    \caption{Diagram of the laser beam reflecting off the cantilever surface. The deflection of the cantilever as the tip moves across the surface of the sample is measured by the deflection of the beam reflecting onto the photodetector. The signal from the photodetector is used to adjust the position of the cantilever to recenter the beam.}
...
\begin{itemize}
    ...
    \item Interaction with the surface allows for measurements of physical properties of a surface.

    \item The sample needs not be conductive (compare to STM).
...
The piezoelectric effect is very useful within many applications that involve the production and detection of sound, generation of high voltages, electronic frequency generation, micro-balances, and ultra-fine focusing of optical assemblies. It is also the basis of a number of scientific instrumental techniques with atomic resolution, such as scanning probe microscopes (STM). The piezoelectric effect also has its use in more mundane applications as well, such as acting as the ignition source for cigarette lighters. 

\subsubsection{Detector and Feedback Loop}
...
The sample stage scans around in a raster pattern underneath the probe. When the probe encounters a surface feature (a bump or whatever), the cantilever bends. This changes the position of the laser beam at the photodetectors. The T-B signal is recorded by the software. In ``feedback mode,'' the T-B signal is an error signal applied to the sample stage's z-piezo. The feedback mechanism works to keep the deflection of the cantilever at a minimum. In this case, the feedback signal gives all of the information about the surface.
...
\textbf{Note: If you are experiencing bugs in the software, refer to the} \hyperref[sec:Troubleshooting]{Troubleshooting Section} \textbf{at the end of this lab}

The software controls the AFM mechanisms and exports collected data as a .wsf file. In particular, it controls the motion of the sample stage, automates the tip approach, measures the resonance frequency of the cantilever for ``vibrating mode'' (see below), and collects the data from the photodetector.

Use this section as a reference when you calibrate the AFM and scan.




\subsubsection{Prescan Tab} Where you select a mode (if vibrating mode, you will also find the resonant frequency), perform a \hyperref[RangeCheck]{range check}, and engage the \hyperref[subsec:TipApproach]{tip with the sample}.
...
\subsubsection{Topo Scan Tab} \label{subsubsec:TopoScanTab} 
        Where you can adjust scan parameters like scan speed, scan size, what measurements you want to take (i.e. Z Drive will give basic topography, Z phase will give hardness, and Z error will map abrupt changes in topography). A screenshot of the tab is shown below:
...
\subsubsection{System Tab} Where you will adjust parameters for resolution, minimizing error, and most importantly \textbf{NOT} \hyperref[subsec:BrokenTip]{BREAKING THE TIP}!

\begin{figure}[h]
\centering
    \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/systemtab.JPG}{\includegraphics[width=0.75\linewidth]{images/systemtab.JPG}}
    \caption{System Tab}
\end{figure}
...
\subsection{Turning on instruments and opening programs}
...
\begin{itemize}
    ...
    \begin{figure}[H]
        \centering
        \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/12.JPG}{\includegraphics[width=0.75\linewidth]{images/12.JPG}}
        \caption{Grounding Plug}
    \end{figure}
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        \centering
        \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/ProbeHolderExchange.png}{\includegraphics[width=0.35\linewidth]{images/ProbeHolderExchange.png}}
    ...
    \begin{figure}[H]
        \centering
        \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/17.JPG}{\includegraphics[width=0.6\linewidth]{images/17.JPG}}
    ...
    \begin{figure}[H]
        \centering
        \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/19.JPG}{\includegraphics[width=0.65\linewidth]{images/19.JPG}}
        \caption{Extracted Probe}
    \end{figure}
    ...
    \begin{figure}[H]
        \centering
        \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/20.JPG}{\includegraphics[width=0.65\linewidth]{images/20.JPG}}
        \caption{Use tweezers to gently pick up new probes.}
        \label{fig:ProbeBox}
    \end{figure}
    ...
    \centering
         \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/AFMprobe.JPG}{\includegraphics[height=180pt]{images/AFMprobe.JPG}}
        \caption{AFM probe bottom view}
        \end{minipage}\hfill
    \begin{minipage}{0.59\linewidth}
        \centering
        \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/21.JPG}{\includegraphics[height=180pt]{images/21.JPG}}
        \caption{Place the new probe in the oval cup}
    ...
    \begin{itemize}
        ...
        \item Recommended to watch tutorial video \href{http://experimentationlab.berkeley.edu/sites/default/files/gettingstarted\_final2.mp4}{\textbf{Getting Started and Probe Exchange}} for this step.
    \end{itemize}
    ...
    \begin{figure}[H]
        ...
        \caption{Carefully tap the exchange tool to slide the probe into the clip.}
        \label{fig:22}
    \end{figure}
...
\begin{enumerate}
    \item First, raise the tip at least 2 cm away from the scanner to give yourself lots of room to work with.
    ...
    \textbf{DO NOT LOSE THESE SCREWS. KEEP THEM WITH THE SCANNER.}

    \item CAREFULLY, unplug the ribbon cable from the back of the scanner. Use caution when removing the cable. \textbf{Grip the cable from the connector when removing to not damage the cable.}
...
\subsection{Centering Probe over Scanning Area/Sample Holder}
\label{subsec:CenteringProbeOverScanningAreaSampleHolder}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Refer to \hyperref[subsec:HowToLoadASample]{How to Load a Sample Section} located above.

        \item Adjust the Camera Zoom to zoom out, so you can determine the orientation of the slide using the Camera Viewer.
    \end{itemize}

    \begin{itemize}
        \item Refer to the \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/Reference-\%20sample-SHS-01\_3\_datasheet.pdf}{\textbf{datasheet}}  for the schematics of the reference sample, and the dimensions of its features.
    \end{itemize}
    ...
    \item Position a clean portion of the desired feature (refer below) on the reference sample under the tip of the cantilever. Please refer to the \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/Reference-\%20sample-SHS-01\_3\_datasheet.pdf}{\textbf{datasheet}} to identify where each feature is located on the reference sample.
        \begin{itemize}
...
\begin{enumerate}
    ...
    \item Set \textbf{Scan Mode} to \textbf{Vibrating} mode, so the \textbf{Tune Frequency} window becomes enabled. 
    ...
    \item Perform a \textbf{range check} \label{RangeCheck} (refer to \href{http://experimentationlab.berkeley.edu/sites/default/files/prescan\_final2.mp4}{\textbf{Pre-Scan: Tune Frequency, Tip Approach, and Scanning}}) if the indicator light is off. 
    ...
    \item Tune the Frequency and Phase (refer to  \href{http://experimentationlab.berkeley.edu/sites/default/files/prescan\_final2.mp4}{\textbf{Pre-Scan: Tune Frequency, Tip Approach, and Scanning}}):
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \item On the upper oscilloscope (Amplitude v. Frequency), move green and red slider bars closer to the resonance peak. This adjusts the sweep range
            ...
            \begin{figure}[H]
                \centering
                \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/FreqPhase.JPG}{\includegraphics[width=0.8\linewidth]{images/FreqPhase.JPG}}
                \caption{Tune Frequency Tab}
            \end{figure}
...
\checkpoint{Tip Settings}{Call a GSI over and show them the settings you have selected here prior to performing a tip approach. Discuss what these settings do, and why you chose them.}
...
NOTE: For non-vibrating mode, make sure you have ``\emph{Non-Vibrating''} selected under \emph{Scan Mode} in the Pre Scan Tab.  The rest of the tip approach process is the exact same as Vibrating Mode, except that you will not need to do a \emph{Tune Frequency} because the probe will not be vibrating in this mode.
...
\begin{enumerate}
    ...
    \item Speed of motor is adjustable with the slider bar. It is advised that the slider bar be set all the way to the left at the slowest setting.  The indicator will read zero, but the motor will still move (should be able to hear the motors activate).
    ...
    \begin{figure}[H]
        ...
        \centering
        \caption{Approximately how focused the probe should \\ be before starting an automated tip approach}
    \end{figure}
...
You are now ready to begin the calibration scan. 
    \begin{enumerate}
        ...
        \item Set parameters and settings to match those in the screenshot below. You may play around with these settings to see how the scan quality changes, or use your own prefered settings. Make sure to record the settings used for your own reference.
        \begin{itemize}
        ...
        \item\textbf{**TO SAFELY DISENGAGE THE TIP, CLICK THE UP BUTTON IN THE TIP RETRACT SECTION**}. When the tip retract completes, the green indicator light will turn on.
        
        \item \textbf{The scanning portion of the system calibration is now complete.}
...
\checkpoint{Laser Alignment}{With the camera, show the staff the alignment of the laser on the cantilever, as well as the alignment of the sample stage. Show the staff the alignment of the detector as well. Show the staff your resonance data, as well as the peak you have chosen. Show the staff your image. Identify any debris or dirt.}
...
\label{subsec: DataAnalysisPortion}
...
Watch the \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/gwyddion1.mp4.mp4}{\textbf{Video tutorial for Gwyddion here}}
...
\textbf{Note to \LabEngineer: The original Gwyddion folder is buried deeper in this computer, in case the one located above is accidentally corrupted.}
...
\begin{itemize}
    ...
    \begin{figure}[H]
        ...
        \caption{Artifacts and Contaminants}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \begin{figure}[H]
                \centering
                \href{http://experimentationlab.berkeley.edu/sites/default/files/AFMImages/47.png}{\includegraphics[width=0.8\linewidth]{images/47.png}}
                \caption{Vertical Line Leveling}
            \end{figure}
...
\emph{\textbf{In this lab you will perform THREE of the following experiments in addition to the REQUIRED EXPERIMENTS: Measuring the Noise floor (which we recommend you do first) and Measuring Boltzmann's Constant (which we recommend you do last) (5 experiments total).}}

\begin{enumerate}

\item \hyperref[subsec:NoiseFloor]{Noise Floor Experiment} -- \textbf{Required}
...
\item \hyperref[subsec:Boltzmann]{Spring Constant of the Cantilever and Measuring Boltzmann's Constant} -- \textbf{Required}

\end{enumerate}

\checkpoint{Chosen Experiment}{After you have completed it, choose one of the non-required experiments here and give a detailed explanation of what you did and what you hope to find out by analyzing your scan data in Gwyddion or otherwise. If the experiment calls for a calculation, give some indication of how you'll determine error.}
...
This short REQUIRED experiment is designed to show you how your physical behavior and environmental variables will affect the outcome of your measurements.
...
\checkpoint{Noise Floor}{The primary purpose of finding the noise floor is to calculate one of the principal sources of error in this experiment. Discuss with a staff member how you can use your newly found knowledge of the noise floor to improve future scans.}
...
\begin{enumerate}
    ...
    \item You will begin by preparing your own CD samples from the provided disks (The DVD samples are already mounted, but you'll have to remove the blueish plastic covering before scanning). This is because after the foil containing the encoded information is removed from the protective plastic disk it will quickly (in about a week) oxidize and become largely unable to be scanned. 
    
    \begin{enumerate}
    
    \item Along with the sample containers, there should also be 2 optical disks clearly labeled as "blank CD" and "CD with data". Additionally, there should be a set of magnetic sample plates labeled for use with the CDs, and a small plastic bag with silicon wafers labeled "SiO$_2$ Chip Mounts".
    ...
    \item Lay out a paper towel on the table and work exclusively on it (To catch any flakes that come off the CD and to protect the table). 
    ...
    \item Take two silicon wafers out of the bag and attach them firmly to the two magnetic plates so that their shiny sides faces upwards, then repeat the above step to put tape on the silicon wafers.
    
    \begin{figure}[H]
    \captionsetup{justification=centering}
         \begin{minipage}[t]{.33\linewidth}
        \centering
        \href{http://experimentationlab.berkeley.edu/sites/default/files/SampleMaterials.JPG}{\includegraphics[height=0.75\linewidth,keepaspectratio]{images/SampleMaterials.JPG}}
        \caption{Sample Materials}
    \end{minipage}
    \begin{minipage}[t]{.33\linewidth}
        \centering
        \href{http://experimentationlab.berkeley.edu/sites/default/files/CuttingCD.JPG}{\includegraphics[height=0.75\linewidth,keepaspectratio]{images/CuttingCD.JPG}}
        \caption{Cutting the CD}
    \end{minipage}
    \begin{minipage}[t]{.33\linewidth}
        \centering
        \href{http://experimentationlab.berkeley.edu/sites/default/files/FinalSample.JPG}{\includegraphics[height=0.75\linewidth,keepaspectratio]{images/FinalSample.JPG}}
        \caption{Final Prepared Sample}
    \end{minipage}
    \end{figure}
    
    \item Take the blank CD and lay it on the paper towel shiny side DOWN (The underside of a CD is a thick plastic disk that we will NOT be cutting through). Using the X-Acto knife, cut a small (smaller than the silicon wafer) square out of the top of the CD, and use the tweezers to attach it to the silicon wafer shiny side UP. Be gentle here, as you don't want to contaminate or wrinkle the surface of the foil, but it needs to be well connected to the silicon wafer or your later scans will be fuzzy. Repeat this step for the other disk. \textbf{Make sure you label or otherwise keep track of which sample is which!}
    ...
    \item Scan your CD and DVD samples using the same scan size for each. Remove the covering from the DVD samples, and replace it once you've completed scanning. Save the data/image.
...
\checkpoint{Boltzmann's Constant}{Show a GSI the calculation you used to determine Boltzmann's constant and indicate any sources of error you might have.}
