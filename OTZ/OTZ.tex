...
\begin{enumerate}
    ...
    \item With the trapping laser on and the emission filter off (in the run wheel), move the objective up and down to find the reflection of the laser on the coverslip and the slide. Beginning with the focus of the laser below the microscope stage, move the objective up until the focus coincides with the top of the coverslip and the first reflection can be seen. As you move the objective further up, the reflection will disappear and then reappear when the focus is on the bottom of the slide. The beads are suspended between these two heights, perhaps close enough to the slide or coverslip so that the laser’s reflection can be seen when the beads are in focus. 
    
    Focus again on the coverslip, and then search for beads floating above it. Keep track of the position of the objective relative to the coverslip. The lens should never be so low that the immersion oil fails to fill the gap, nor so high that it pushes the slide up off the stage. The beads should be in focus, and moving around. If they are stationary, you may be focused on beads that have sunk to the bottom. Raise the objective to search for beads suspended above the coverslip. If you have verified that all the beads have sunk to the bottom, you will need to prepare a new slide. If beads are scarce, you may need to move the stage horizontally with the joystick to find some.  
...
\checkpoint{Trapping a Single Bead}{Call over a GSI or Professor and show them that you have successfully focused an image of the silica solution and that you are properly trapping a single bead. Also show that you have centered the QPD around zero on the graph display.}
...
\begin{enumerate}
    ...
    \item Set a sample rate of 1kHz and history length of 8 s (or however long you need to capture the data). You can ignore most of the other settings for now.
    ...
    \item Focus the microscope so that the bead is slightly out of focus in the same direction and by the same magnitude as if it were a freely suspended, trapped bead. Use your prior experience trapping to judge this. 
    ...
    \item Once the bead is centered, switch to software control of the stage, set the number of steps (labeled ``step size" in the GUI) to at least 100, and the frequency to about 50-150 Hz.
...
\checkpoint{Sensitivity vs Laser Power Plot}{Call over a GSI or Professor and show them the plot you have made of sensitivity vs laser power for X and Y. Explain how the sensitivities varies with power and what is happening.}
...
\checkpoint{Sensitivities From Alpha Values}{Call over a GSI or Professor and show them your graphs of $\rho_x$ , $\rho_y$, $k_x$, $k_y$ versus laser power as well as your solutions for $x$ and $y$ sensitivities from the alpha values $a_x, a_y$.}
...
\checkpoint{Stiffness Values}{Call over a GSI or Professor, show them your values for stiffness and discuss your answers to the following questions:}
...
\checkpoint{Two Sets of Questions}{Complete at least two sets of questions. Call over a Professor or GSI and discuss the questions and the methods you will be using to tackle them.}
